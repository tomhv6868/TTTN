...
usage() {
    printf 'usage: bash scripts/ubuntu_t85_live_attacks.sh --run-id ID --attack-id hping3|ftp-patator [--bundle DIR]\n' >&2
    exit 2
}
...
while (($#)); do
    case "$1" in
        ...
        --attack-id) [[ $# -ge 2 ]] || usage; ATTACK_ID="$2"; shift 2 ;;
...
case "$ATTACK_ID" in
    hping3|ftp-patator) ;;
    *) printf 'error: --attack-id must be hping3 or ftp-patator\n' >&2; exit 2 ;;
esac
...
readonly RUN_ID ATTACK_ID
readonly ATTACK_ROOT="$PROJECT_ROOT/run_log/t8.5/live-attacks/$RUN_ID/$ATTACK_ID"
readonly RUNTIME_ROOT="$ATTACK_ROOT/ubuntu-runtime"
...
readonly DETECTION_LOG="$ATTACK_ROOT/detection.jsonl"
...
for evidence in \
    ...
    "$DETECTION_LOG"; do
    [[ ! -e "$evidence" ]] || {
        printf 'error: live evidence already exists; preserve it and use a new --run-id: %s\n' "$evidence" >&2
        exit 1
    }
done
...
printf '[NIDS] live run: %s\n' "$RUN_ID"
printf '[NIDS] attack: %s (dedicated process; flow state starts empty)\n' "$ATTACK_ID"
...
sudo env LD_LIBRARY_PATH="$LD_LIBRARY_PATH" "$BINARY" \
    ...
    --file-prefix=nids-t85-live --huge-unlink=always --no-telemetry \
    '--log-level=*:warning' \
