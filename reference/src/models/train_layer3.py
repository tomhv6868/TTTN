"""
train_layer3.py — Layer 3 LightGBM trainer (Session-level classifier).

Trained on UNSW-NB15 + Bot-IoT NF. Session-level features include TTL,
load, connection-tracking stats, and state — features that Zeek conn.log
naturally provides at runtime.

Output:
  models/layer3_lgbm.pkl
  models/layer3_scaler.pkl
  data/processed/layer3_eval.json
"""
...
from src.pipeline.features import LAYER3_FEATURES  # noqa: E402


def train_layer3(test_size: float = 0.30, random_state: int = 42) -> dict:
    print("=" * 60)
    print(" Layer 3 — LightGBM (Session classifier)")
    print("=" * 60)

    # ── Load ──────────────────────────────────────────────────────────────────
    df = pd.read_parquet(PROC / "layer3.parquet")
    ...
    # ── Export ────────────────────────────────────────────────────────────────
    joblib.dump(model,  MODELS / "layer3_lgbm.pkl")
    joblib.dump(scaler, MODELS / "layer3_scaler.pkl")
    joblib.dump(feature_cols, MODELS / "layer3_features.pkl")
    print("  Saved: models/layer3_lgbm.pkl, layer3_scaler.pkl, layer3_features.pkl")

    results = {
        "layer": 3, "model": "LightGBM",
    ...
    (PROC / "layer3_eval.json").write_text(
